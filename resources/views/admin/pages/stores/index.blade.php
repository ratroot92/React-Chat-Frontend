...
<th>{{$key->category_description}}</th>
<th class=""> <img src="{{ asset(''.$key->category_image) }}" style="display:block;" width="50px" height="50px"></th>
<th>{{$key->category_status}}</th>
...
<a class="btn btn-danger delBtn" href="/admin/del_cat/{{$key->id}}"  ><span class="fa fa-trash"></span></a>
...
<form class="form-horizontal form-label-left" method="POST" action="{{url('/admin/edit_cat_modal')}}"  enctype="multipart/form-data" id="edicategory_form" name="edicategory_form" >
@csrf
...
$.ajax({
type:'get',
url:'/admin/get_cat_for_edit/'+id,
data:{id:id},
success:function(response){

$('#id').val(response.id);
$('#name').val(response.category_name);
$('#description').val(response.category_description);
var status=response.category_status;
if(status=='1'){
$('#status').val("1");
}
else{
