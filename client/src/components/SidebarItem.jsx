import React from "react";
import socketClient from "../config/socket.config";
import { useAuth } from "../context/AuthContext";
import { useChat } from "../context/ChatContext";
const avatarSource =
  "data:image/jpeg;base64,";

function SidebarItem({ chat }) {
  const { chatState, setSelectedChat } = useChat();
  const {authState}=useAuth()
  const [online,setOnline]=React.useState([])


  React.useEffect(() => {
    if(chatState?.selectedChat?._id ){
      if(chatState.selectedChat._id===chat?._id){
        socketClient.emit("joinChatRoom",{chat,joiner:authState?.user})
      }
    }


    function joinChatRoom({joiner}){
      // // console.log(authState?.user?._id,joiner?._id)
      if(joiner?._id!==authState?.user?._id){
        console.log("joinChatRoom ",`${joiner?.name} has joined the chat room.`)
        if(!online.includes(joiner?._id)){
          // const clone=[...online];
          // clone.push(joiner?._id)
          setOnline([...online,joiner?._id])

        }
      }
    }
    socketClient.on("joinChatRoom", joinChatRoom);

    return () => {
      socketClient.off("joinChatRoom", joinChatRoom);
    };
  }, []);


  React.useEffect(()=>{
    if(online.length){
      console.log("online",online)
    }

  },[online])




  return (
    <div
      key={chat._id}
      className={`col-md-12 mt-1 mb-1  border border-${
        chatState?.selectedChat?._id === chat?._id ? "success" : "dark"
      }`}
      style={{ borderRadius: "3px" }}
      onClick={() => {
        if (socketClient.connected) {
          // socketClient.emit("joinChatRoom", `${chatState?.selectedChat?._id}`);
        }
        if (chatState.selectedChat._id !== chat._id) {
          setSelectedChat(chat._id);
          // socketClient.emit("joinChatRoom",{chat,joiner:authState?.user?._id})

        }
      }}
    >
      <div className="row">
        <div className="col-sm-12 col-md-12 col-lg-3 d-flex flex-row justify-content-center align-items-center">
          <img
            src={avatarSource}
            height={45}
            width={45}
            alt=""
            style={{ borderRadius: "50%" }}
            className={`border border-${
              chatState.selectedChat._id === chat._id ? "success" : ""
            }`}
          />
        </div>
        <div
          className={`col-md-12 col-lg-9  bg-${
            chatState?.selectedChat?._id === chat?._id ? "" : ""
          }`}
        >
          <div className="row  ">
            <div
              className="col-md-12   d-flex flex-column justify-content-center align-items-start"
            >
              <p
                style={{ fontSize: "12px",fontWeight:"bold" }}
                className={`p-0 m-0 ${
                  chatState.selectedChat._id === chat._id ? "text-success" : ""
                }`}
              >
                {chat?.users?.filter(u=>u._id!==authState?.user?._id)[0]?.name}
              </p>
              {chat?.lastMessage ? (
                <>
                <p
                className={`p-0 m-0 ${
                  chatState.selectedChat._id === chat._id ? "text-success" : ""
                }`}
                  style={{
                    fontSize: "12px",

                  }}
                >

                  {chat?.lastMessage?.content}   @{chat?.lastMessage?.sender?.name}
                </p>
                <p  style={{
                    fontSize: "12px",

                  }}>
                  {online&& online.includes(chat?.users?.filter(user=>user._id!==authState?.user?._id)[0]?._id) || online.includes(chat?.users?.filter(user=>user._id!==authState?.user?._id)[1]?._id)?"Online":"Offline"}
                </p>
                </>

              ) : (
                <p
                className={`p-0 m-0 ${
                  chatState.selectedChat._id === chat._id ? "text-success" : ""
                }`}
                  style={{
                    fontSize: "12px",
                    // fontFamily:"sans-serif"
                    fontFamily: `'Times New Roman', Times, serif`,
                  }}
                >
                  No Message History
                </p>
              )}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default SidebarItem;
