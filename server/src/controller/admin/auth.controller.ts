...
const adminAuthController = {
  async login(request: Request, response: Response, next: NextFunction): Promise<any> {
    try {
      passport.authenticate('admin-login', (err: any, user: IUserDoc, info: IVerifyOptions) => {
        if (err) {
          return next(err);
        }
        if (!user) {
        ...
        const accessToken = jwt.sign({ iss: process.env.JWT_ISSUER, sub: user }, process.env.JWT_ADMIN_SECRET, { expiresIn: '1d' });
